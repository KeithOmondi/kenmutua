...
const Gallery = () => {
  ...
  const lightboxItem = items.find((g) => g.id === lightbox) ?? null;
  ...
  const renderContent = () => {
    if (isLoading) {
      return (
        <div className="text-center py-20 text-[#7A4A2E] font-light italic">
          Loading gallery...
        </div>
      );
    }
    ...
    if (error) {
      return (
        <div className="text-center py-20 max-w-sm mx-auto">
          <p className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-xl px-6 py-4">
            ⚠️ Could not load gallery. Please try again later.
          </p>
        </div>
      );
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {filtered.map((item) => (
          ...
          >
            <div className="relative aspect-[4/3] overflow-hidden bg-[#E8D5C0]">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[rgba(44,26,14,0.9)] via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 translate-y-5 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 text-white">
                  <div className="font-['Cormorant_Garamond',serif] text-lg font-bold mb-1">
                    {item.label}
                  </div>
                  <div className="text-sm font-light opacity-90 leading-relaxed mb-2">
                    {item.caption.substring(0, 60)}...
                  </div>
                  <div className="text-xs font-normal opacity-70 flex items-center gap-1">
                    📍 {item.location}
                  </div>
                </div>
              </div>
            ...
            <div className="p-5">
              <div className="font-['Cormorant_Garamond',serif] text-lg font-bold text-[#2C1A0E] mb-2">
                {item.label}
              </div>
              <div className="text-sm font-light text-[#7A4A2E] leading-relaxed mb-2">
                {item.caption.substring(0, 80)}...
              </div>
              <div className="text-xs font-normal text-[#3E6B4E] flex items-center gap-1">
                📍 {item.location}
              </div>
            </div>
          </div>
  ...
  return (
    <>
      ...
      <section className="bg-[#FAF5EC] py-24 px-6 lg:px-12 font-['Outfit',sans-serif]" id="gallery">
        ...
        {/* Filter tabs — only show when items are loaded */}
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`
                  ...
                  ${active === cat
                    ? "bg-[#3E6B4E] text-white shadow-md shadow-[rgba(62,107,78,0.3)]"
